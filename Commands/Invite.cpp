...
void ft_invite(commandInfo& cmd, Server& server, Client& client) {
	...
	{
		server.sendReply(ERR_USERONCHANNEL(std::string("*"), target->second.nickname, target->second.realname, (*channelIter)->getName()), client.fds.fd);
		return ;
	}
	...
	server.sendReply(RPL_CUSTOM_INVITE(setPrefix(server.hostname, client.nickname, client.realname), cmd.cmnd_args[0], (*channelIter)->getName()), target->second.fds.fd);
}
