...
void ft_notice(commandInfo& cmd, Server& server, Client& client) {
    ...
    if((cmd.cmnd_args[0][0] == '%' && cmd.cmnd_args[0][1] == '#') || cmd.cmnd_args[0][0] == '#')
    {
        ...
        if (!(*channel)->isJoined(client.nickname))
            return;
        (*channel)->broadcastMessage(&client, RPL_CUSTOM_NOTICE(setPrefix(server.hostNames[client.fds.fd], client.nickname, client.realname), channelName, cmd.cmnd_args[1]), opeOnly);
    }
    else
    {
        ...
        if (Receiver  == server.users.end())
            return;
        server.sendReply(RPL_CUSTOM_NOTICE(setPrefix(server.hostNames[client.fds.fd], client.nickname, client.realname), Receiver->second.nickname, cmd.cmnd_args[1]), Receiver->second.fds.fd);
    }
}